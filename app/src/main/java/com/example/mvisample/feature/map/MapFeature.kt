...
class MapFeature @Inject constructor(
    ...
    sealed class MapEffect {
        ...
        data class MapMoving(val title: String) : MapEffect()
        data class ChangeBottomViewState(val newState: BottomState) : MapEffect()
    ...
    sealed class MapAction {
        ...
        data class ChangeBottomViewState(val newState: BottomState) : MapAction()
    ...
    data class MapState(
        ...
        val viewState: BottomState = BottomState.HIDDEN
    )
    ...
    class MapReducer : Reducer<MapState, MapEffect> {
        override fun invoke(state: MapState, effect: MapEffect): MapState {
            return when (effect) {
                ...
                is MapEffect.PointsLoaded -> state.copy(points = effect.location.map { Point(it) })
                ...
                is MapEffect.MapMoving -> state.copy(
                    title = effect.title,
                    viewState = BottomState.HIDDEN
                )
                is MapEffect.ChangeBottomViewState -> state.copy(viewState = effect.newState)
            }
        }
    }
